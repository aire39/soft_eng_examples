...
class EmbraerFactory : public AbstractFactory
{
    public:
        EmbraerFactory(const EmbraerFactory&) = delete;
        void operator=(const EmbraerFactory&) = delete;
        ~EmbraerFactory() = default;

        static AbstractFactory* instance()
        {
            static EmbraerFactory factory;
            return &factory;
        }
